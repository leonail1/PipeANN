...
- A probe that lowered libcuckoo's global default initial table size had negligible single-query RSS impact, because the current single-query path already uses local `(0)` construction for empty tag/cache/lock tables. The global default change was not adopted because it can slow unrelated large concurrent hash maps.
- The committed direction is local sizing: pass `(0)` for known-empty maps, and pass estimated live/delete cardinalities for dynamic merge maps.
- The single-query runner now reads only the first query vector from the query file. This makes the resource test measure a true single-query invocation instead of retaining the full 1000-query buffer.
