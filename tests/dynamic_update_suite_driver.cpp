...
Config parse_args(int argc, char **argv) {
  ...
  if ((config.mode == "measure-dynamic-search" || config.mode == "search-during-insert") && config.query_bin.empty()) {
    throw std::runtime_error("--query-bin is required for search modes");
  }
  return config;
}
